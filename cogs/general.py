...
def is_admin(ctx) -> bool:
    if int(ctx.author.id) in cfg.id_owner:
        return True
    for role in ctx.author.roles:
        if cfg.role_admin == role.id:
            return True
    return False
...
class General(commands.Cog, name="General"):
    def __init__(self, bot):
        ...
        self.existing_roles = ["mission pooler", "admins"]
    ...
    async def reply(self, ctx, msg_type: str, message: str, delete_after: int = 30):
        ...
        if delete_after:
            if msg_type in self.msg_type_emoji:
                await ctx.reply(
                    f"{self.msg_type_emoji[msg_type]} {message}",
    ...
    async def whoami(self, ctx: Context):
        ...
        embed = discord.Embed(
            title=f"{ctx.author.name}{f'#{ctx.author.discriminator}' if int(ctx.author.discriminator) > 0 else ''}",
            timestamp=datetime.now(),
    ...
    async def guess(self, ctx, number: int = 10):
        ...
        """
        await ctx.send(
            f"Devine un nombre entre 1 et {number}. Tu as 10 secondes.", delete_after=20
        )
        ...
        try:
            ...
            await ctx.send(
                f"Désolé, tu as mis trop de temps. La réponse était {answer}.",
                delete_after=10,
            )
    ...
    async def lance(self, ctx, n: str = "1", number: int = 20):
        ...
        color = (
            min(answer) == 1
            and 0xFF0000
            or (max(answer) == number and 0x00FF00 or 0x777777)
        )
    ...
    async def delete_dms(self, ctx):
        ...
        if ctx.guild:
            await ctx.reply(
                "Cette commande ne peut être utilisée que dans les DMs", delete_after=10
            )
            await sleep(10)
            await ctx.message.delete()
            return
        ...
        msg_history_list = [
            message async for message in ctx.channel.history(limit=1000)
        ]
        msg_history_list = (
            ctx.clean_prefix == "/" and msg_history_list[1:] or msg_history_list[2:]
        )
        ...
        self.bot.logger.info(
            f"Suppression de {len(msg_history_list)} message(s) dans les DMs de {ctx.author.display_name}"
        )
